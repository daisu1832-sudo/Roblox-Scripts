...
local LagKickTab = Window:MakeTab({
    Name = "💀 ラグキック",
    Icon = "",
    PremiumOnly = false
})
...
--==================================================
-- 💀 ラグキック
-- 高さモード（Spawn / Heaven）は移植しない
--==================================================

local LagKickPlayers = game:GetService("Players")
local LagKickReplicatedStorage = game:GetService("ReplicatedStorage")
local LagKickWorkspace = game:GetService("Workspace")
local LagKickLocalPlayer = LagKickPlayers.LocalPlayer
local LagKickGrabEvents = LagKickReplicatedStorage:FindFirstChild("GrabEvents")

local lagKickLineEnabled = false
local lagKickLineThread = nil

local function lagKickGetTargets()
	local result = {}
	for _, plr in ipairs(LagKickPlayers:GetPlayers()) do
		if plr ~= LagKickLocalPlayer then
...
local function lagKickSetOwner(hrp)
	if not LagKickGrabEvents or not hrp then return end
	local remote = LagKickGrabEvents:FindFirstChild("SetNetworkOwner")
	if remote then
		pcall(function()
			remote:FireServer(hrp, hrp.CFrame)
		end)
	end
end

local function lagKickTeleportNear(myHrp, targetHrp)
...
local function lagKickDestroyLine(hrp)
	if not LagKickGrabEvents or not hrp then return end
	local createLine = LagKickGrabEvents:FindFirstChild("CreateGrabLine")
	local destroyLine = LagKickGrabEvents:FindFirstChild("DestroyGrabLine")
...
local function lagKickStartLineLag()
	if lagKickLineEnabled then return end
	lagKickLineEnabled = true

	lagKickLineThread = task.spawn(function()
		if not LagKickGrabEvents then
			lagKickLineEnabled = false
			return
		end
		...
		while lagKickLineEnabled do
			local spawnLocation =
				LagKickWorkspace:FindFirstChild("SpawnLocation")
				or LagKickWorkspace:FindFirstChild("Spawn")
				or (LagKickLocalPlayer.Character and LagKickLocalPlayer.Character:FindFirstChild("HumanoidRootPart"))
			...
			if spawnLocation then
				...
				for _, pos in ipairs(directions) do
					if not lagKickLineEnabled then break end
					pcall(function()
...
local function lagKickStopLineLag()
	lagKickLineEnabled = false
	lagKickLineThread = nil
end

LagKickTab:AddButton({
	Name = "ラグキック開始",
	Callback = function()
		task.spawn(function()
			lagKickStartLineLag()
			...
			local players = lagKickGetTargets()
			if #players == 0 then
				lagKickStopLineLag()
				OrionLib:MakeNotification({
					Name = "ラグキック",
					Content = "対象プレイヤーがいません",
					Time = 3
				})
				return
			end

			local myChar = LagKickLocalPlayer.Character
			...
			for _, data in ipairs(playerData) do
				lagKickTeleportNear(myHrp, data.hrp)
				task.wait(0.2)
				lagKickSetOwner(data.hrp)
				task.wait()
			end
	...
	for _, object in ipairs(gui:GetDescendants()) do
		if (
			...
			if object.Text == "🚀 ロケット"
				...
				or object.Text == "💀 ドリフトキック"
				or object.Text == "💀 ラグキック" then
				object.TextSize = 22
				object.Position =
					object.Position
